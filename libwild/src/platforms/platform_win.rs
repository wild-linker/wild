...
pub(crate) mod fs {
    ...
    pub(crate) fn read_input(file: &File, path: &Path, prepopulate: bool) -> Result<InputBytes> {
        crate::platforms::common::map_input(file, path, prepopulate)
    }
...
pub(crate) mod linker_plugin {
    use crate::error::Result;
    pub(crate) use crate::platforms::common::UnsupportedPluginLibrary as PluginLibrary;
    use std::ffi::c_int;
    use std::fs::File;

    /// The linker plugin API passes inputs as file descriptors, which Windows doesn't have.
    pub(crate) const SUPPORTED: bool = false;

    pub(crate) type OffT = std::ffi::c_long;
...
pub(crate) mod process {
    pub(crate) use crate::platforms::common::no_fork::CAN_FORK;
    pub(crate) use crate::platforms::common::no_fork::ParentNotifier;
    pub(crate) use crate::platforms::common::no_fork::fork_linker;
}
